...
router.post("/start/:taskId", authenticate, async (req,res)=>{
    try{
        const task = await Task.findOne({_id:req.params.taskId, userId:req.id}).lean().exec()
        if(!task){
            return res.status(404).send({message:"Task not found"})
        }
        const active = await TimeLog.findOne({taskId:req.params.taskId, userId:req.id, endTime: null}).lean().exec()
        if(active){
            return res.status(400).send({message:"Tracking already active"})
        }
        const log = await TimeLog.create({taskId:req.params.taskId, userId:req.id, startTime:new Date()})
...
router.post("/stop/:taskId", authenticate, async (req,res)=>{
    try{
        const active = await TimeLog.findOne({taskId:req.params.taskId, userId:req.id, endTime: null}).sort({startTime:-1})
        if(!active){
...
// List logs for a task + total duration (ms)
router.get("/task/:taskId", authenticate, async (req,res)=>{
    try{
        ...
        const logs = await TimeLog.find({taskId:req.params.taskId, userId:req.id}).sort({startTime:-1}).lean().exec()
        const now = Date.now()
